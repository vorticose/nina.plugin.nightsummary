...
- Lightbox visual polish: TS Import badge and close button now pin to the image's left and right edges at every viewport size (was anchored to the wider container, drifting away from the image on desktop). Upscaled thumbnails size to their natural aspect ratio so the 3D shadow hugs the actual image content with no letterbox frame on the sides. Image and header re-fit in real time when the browser window is resized. Light mode grading pills (Accepted, Rejected, Pending, Not graded) now use readable dark-text-on-light-tint colors instead of the dark-mode pastels. Consistent 3D depth on the badge, counter, and close button.
