...
**New features**
- Raw image thumbnails (opt-in) — Night Summary can now save a small JPEG thumbnail of every LIGHT frame as it's captured, and surface them in a new dashboard gallery. Three views: per session (from the session detail page), cross-session per target, and per project (when Target Scheduler is installed). The gallery groups thumbnails by target (chronologically, first-imaged at top) then by filter (canonical L/R/G/B/H/S/O/N stack order) and exposure length, with a "Rejected" badge on rejects. Click any thumbnail for a lightbox view with a 4-column metrics panel (Capture, ADU, Guiding, Environment) plus a quick-glance header strip showing date / target / filter / stars / HFR / FWHM / Eccentricity. ADU statistics, focuser/camera temps, airmass and more in the columns; project name, Exposure Profile, and per-axis guiding RMS are pulled from Target Scheduler when available. Smooth slide animation between frames with arrow-key (desktop) or swipe (mobile) navigation; image and metrics reveal together on first open with no layout pop. Optional medium 800px thumbnails (~80 KB each) for sharper lightbox viewing. Off by default — enable in Options → Raw Image Thumbnails. Three retention modes: keep all, roll over by days, or roll over by total disk usage. Custom storage directory option for parking large collections on a different drive. Existing Target Scheduler users can backfill thumbnails from past sessions with a one-click "Import from Target Scheduler" button — typically completes in a few minutes for a thousand frames with live progress and ETA. Thumbnails are stored at `%LOCALAPPDATA%\NINA\NightSummary\thumbs\{sessionId}\` by default.
- Support development — new "Donate" pill in the dashboard header (next to Report a Bug / Suggest a Feature) and a Support link in the plugin description point to https://ko-fi.com/sleepypuppy15 for users who'd like to chip in. GitHub Sponsors also enabled on the repo. The plugin stays free and open-source; donations are always optional. (Closes #26.)
...
- Smoother navigation transitions across the dashboard: back-navigation from a session detail view returns to the Stats page with its TDP/PDP modal re-overlaid in a single paint instead of cascading through visible intermediate states; the TDP "View" button keeps the modal visible until the new session report has painted instead of briefly exposing the page behind it; and cache-hot navigations no longer flash a "Loading…" placeholder.
- Lightbox redesign: the metrics panel renders as elevated stat-box cards (Capture, ADU, Guiding, Environment) with the same depth shadows used by the session cards on the sessions page, plus a quick-glance header strip. Status pill at the left of the strip distinguishes the source: "TS Accepted/Rejected/Pending" (from Target Scheduler), "Manual Rejected" (from NINA's image grader), or "Not graded" (no grading data). A position counter ("1 / 23") sits at the top of the image; "TS Import" badge marks frames whose source is the small (192px) thumb. Image is wrapped in a subtle 3D border that hugs the actual image edges, with corner radius matching the stat boxes. Background dim plus backdrop blur replaces the old opaque black overlay. Prev/next navigation slides the image and panel as a single unit. Values within each card share a common left edge and right-anchor so columns read cleanly even when widths vary (e.g. the `"` unit on RMS arcsec).
- Lightbox mobile improvements: swipe left/right to navigate between frames (prev/next buttons hidden on small screens), counter shows `‹ 3 / 12 ›` chevrons as a swipe affordance, the metrics panel layout is tightened to fit one phone screen without scrolling, and the header strip compresses to two lines on iPhone-class viewports (was three).
- Lightbox visual polish: TS Import badge, position counter, and close button pin to the image's edges at every viewport size (was anchored to the wider container, drifting away from the image on desktop). Upscaled thumbnails size to their natural aspect ratio so the 3D shadow hugs the actual image content with no letterbox frame on the sides. Image and header re-fit in real time when the browser window is resized. Light mode grading pills now use readable dark-text-on-light-tint colors instead of the dark-mode pastels. Consistent 3D depth on the badge, counter, and close button. File row in the Capture column truncates long filenames with ellipsis; hover reveals the full filename.
- Frames gallery refinements: filter section titles enlarged for easier scanning, per-target and per-filter counts render as matching pill badges, and the redundant per-thumbnail filter caption is removed (the subgroup header already labels each row). Dashboard header credit + Report-a-Bug / Suggest-a-Feature pills bumped slightly more readable while staying secondary chrome.
...
- Frames gallery in sessions whose images were previously TS-graded but where Target Scheduler is no longer installed: thumbnails rendered dimmed with a stale rejection badge for frames the user never actually rejected, and the lightbox claimed "TS Accepted" / "TS Rejected" with no TS to back it. Dashboard now suppresses TS-derived grading/rejection when TS isn't available so frames render normally; genuine NINA-side manual rejections still surface as "Manual Rejected".
