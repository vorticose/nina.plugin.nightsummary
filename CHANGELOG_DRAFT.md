...
- Reports are roughly 8x smaller. The header icon was embedded at full brand-art resolution (776x776, ~600 KB) despite rendering at 48px, which made it about 90% of every report. It is now a 144px copy, so a typical report drops from ~840 KB to ~100 KB. Emailed reports in particular are far less exposed to gateway size limits and truncation.
